...
const initialStudentData = {
    ...
    experience: [
        { role: 'Frontend Intern', company: 'Innovate LLC', duration: 'Jun 2024 - Aug 2024', logo: 'innovate' }
    ],
    education: [
        { degree: 'B.S. in Computer Science', institution: 'State University', year: '2024', logo: 'state-uni' }
    ],
        ...
        <>
            <div className="header-top-row">
                ...
                <div className="header-actions">
                    <button className="action-btn-header btn-secondary-header"><UIIcon name="search" /> Browse Courses</button>
                    <button className="action-btn-header btn-primary-header"><UIIcon name="plus" /> Book a Session</button>
                </div>
            </div>
...
const ProfilePage = ({ student, setStudent }) => {
    ...
    const profileCompletion = Math.round(
        (((formData.headline ? 1 : 0) +
            ...
            (formData.socials.linkedin && formData.socials.github && formData.socials.portfolio ? 1 : 0)
        ) / 6) * 100
    );
    ...
    const renderLogo = (item) => {
        ...
        const style = styles[item.logo] || { background: '#F3F4F6', color: '#4B5563', initials: '?'};
                            ...
                            {formData.experience.map((exp, index) => (
                                <div className="timeline-item" key={index}>
                                    ...
                                    <div className="timeline-details">
                                        {isEditing ? (
                                            <>
                                                <input type="text" className="timeline-input-title" value={exp.role} onChange={(e) => handleInputChange(e, 'experience', index, 'role')} />
                                                <input type="text" className="timeline-input-subtitle" value={exp.company} onChange={(e) => handleInputChange(e, 'experience', index, 'company')} />
                                                <input type="text" className="timeline-input-meta" value={exp.duration} onChange={(e) => handleInputChange(e, 'experience', index, 'duration')} />
                                            </>
                                        ) : (
                                            <>
                                                <h4>{exp.role}</h4>
                            ...
                            {formData.education.map((edu, index) => (
                                <div className="timeline-item" key={index}>
                                    ...
                                    <div className="timeline-details">
                                        {isEditing ? (
                                            <>
                                                <input type="text" className="timeline-input-title" value={edu.degree} onChange={(e) => handleInputChange(e, 'education', index, 'degree')} />
                                                <input type="text" className="timeline-input-subtitle" value={edu.institution} onChange={(e) => handleInputChange(e, 'education', index, 'institution')} />
                                                <input type="text" className="timeline-input-meta" value={edu.year} onChange={(e) => handleInputChange(e, 'education', index, 'year')} />
                                            </>
                                        ) : (
                                            <>
                                                <h4>{edu.degree}</h4>
                ...
                <div className="profile-side-col">
                    ...
                    <div className="profile-card">
                         ...
                         <div className="social-links-container">
                            <a href={formData.socials.linkedin} target="_blank" rel="noopener noreferrer" className="social-link-btn"><UIIcon name="linkedin"/>LinkedIn</a>
                            <a href={formData.socials.github} target="_blank" rel="noopener noreferrer" className="social-link-btn"><UIIcon name="github"/>GitHub</a>
                            <a href={formData.socials.portfolio} target="_blank" rel="noopener noreferrer" className="social-link-btn"><UIIcon name="globe"/>Portfolio</a>
                         </div>
                    </div>
                </div>
            </div>
...
const StudentDashboard = () => {
    ...
    const renderContent = () => {
        switch (activeLink) {
            ...
            case 'Build Resume':
                return <BuildResumePage student={student} />;
            default:
                return <DashboardContent student={student} />;
        }
    };
        ...
        <div className="dashboard-layout">
            ...
            <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''} ${isMobileNavOpen ? 'mobile-open' : ''}`}>
                ...
                <nav className="sidebar-nav">
                    ...
                    <a href="#" className={`nav-link ${activeLink === 'Build Resume' ? 'active' : ''}`} onClick={() => setActiveLink('Build Resume')}><UIIcon name="fileText"/> <span className="nav-text">Build Resume</span></a>
                </nav>
